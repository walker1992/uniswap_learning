// 源码拷自 https://etherscan.io/address/0x1f9840a85d5af5bf1d1762f925bdaddc4201f984#code
// Uni token 合约
...
contract Uni {
     ...
     * 增发代币。这里首先验证最早增发时间并更新。然后验证增发数量不能走过原总量的1/50（先乘于2再除于100）。最后进行增发操作（更新相关地址余额和发行总量），同时转移等量委托（投票）权
     */
     ...
     * 获得某地址当前投票数。看实现逻辑使用了检查点来记录不同时刻的投票数
     */
    ...
    // 委托函数。首先获取旧的委托地址、委托人代币余额，然后更新为新的委托地址。接着触发委托改变事件，最后将相应的委托值（和委托人代币余额数值等额）从旧委托地址转移到新委托地址
    ...
    * 转移代币资产。这个要点有二处：
    * 1、资产的大小为uint96类型，
    * 2、是代币转移完成后转同时委托额度。这里SuShi项目在沿用Compound的治理协议时代币转移函数里就少了最后的委托（投票）转移，成为了一个众所周知的bug
    */
    ...
    // 使用内嵌汇编获取当前以太坊的区块链ID（区块链ID代表它是主链或者测链或者自定义链）。验证线下签名消息时使用，防止在错误的链上验证
